...
export default function StatistikScreen() {
  ...
  const categoryExpenses = [
    { name: 'Food & Dining', amount: 850000, percentage: 35, color: 'bg-orange-500', icon: 'restaurant' },
    { name: 'Transportation', amount: 600000, percentage: 25, color: 'bg-blue-500', icon: 'car' },
    { name: 'Shopping', amount: 480000, percentage: 20, color: 'bg-purple-500', icon: 'bag' },
    { name: 'Bills & Utilities', amount: 320000, percentage: 13, color: 'bg-red-500', icon: 'receipt' },
    { name: 'Entertainment', amount: 170000, percentage: 7, color: 'bg-green-500', icon: 'game-controller' },
  ];
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="px-6 mt-6">
          <View className="flex-row space-x-3">
            <View className="flex-1 bg-white p-4 rounded-2xl shadow-sm">
              ...
              <Text className="text-lg font-bold text-gray-800">Rp {(totalIncome / 1000000).toFixed(1)}M</Text>
              <Text className="text-green-500 text-xs">+12% from last month</Text>
            </View>
            ...
            <View className="flex-1 bg-white p-4 rounded-2xl shadow-sm">
              ...
              <Text className="text-lg font-bold text-gray-800">Rp {(totalExpense / 1000000).toFixed(1)}M</Text>
            ...
            <View className="flex-1 bg-white p-4 rounded-2xl shadow-sm">
              ...
              <Text className="text-lg font-bold text-gray-800">Rp {(totalSavings / 1000000).toFixed(1)}M</Text>
              <Text className="text-blue-500 text-xs">+18% from last month</Text>
            </View>
          </View>
        </View>
        ...
        <View className="px-6 mt-6">
          <View className="bg-white p-6 rounded-2xl shadow-sm">
            <Text className="text-lg font-bold text-gray-800 mb-4">Monthly Overview</Text>
              ...
              {monthlyData.map((data, index) => {
                ...
                return (
                  <View key={index} className="space-y-2">
                    <Text className="text-sm font-medium text-gray-600">{data.month}</Text>
        ...
        <View className="px-6 mt-6">
          <View className="bg-white p-6 rounded-2xl shadow-sm">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-lg font-bold text-gray-800">Expense Categories</Text>
              <TouchableOpacity>
            ...
            <View className="mb-6">
              <Text className="text-center text-sm text-gray-600 mb-4">Category Distribution</Text>
              <View className="space-y-3">
                {categoryExpenses.map((category, index) => (
                  <View key={index} className="flex-row items-center">
                    <View className={`${category.color} w-3 h-3 rounded-full mr-3`} />
                ...
                <View key={index} className="flex-row items-center justify-between">
                  <View className="flex-row items-center flex-1">
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-sm text-gray-500">{category.percentage}% of total</Text>
                    </View>
                  </View>
        ...
        <View className="px-6 mt-6 mb-8">
          <View className="bg-gradient-to-r from-blue-500 to-purple-600 p-6 rounded-2xl">
            <View className="flex-row items-center mb-3">
              ...
              <Text className="text-white font-bold text-lg ml-2">Smart Insights</Text>
            </View>
            <Text className="text-white opacity-90 text-sm leading-6">
              Your food expenses increased by 15% this month. Consider setting a budget limit of Rp 750,000 for dining to maintain your savings goal.
            </Text>
            <TouchableOpacity className="bg-white bg-opacity-20 p-3 rounded-xl mt-4">
              <Text className="text-white font-medium text-center">Set Budget Alert</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
